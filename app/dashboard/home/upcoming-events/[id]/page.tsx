...
const EditSliderPage = () => {
  ...
  useEffect(() => {
    const fetchSlider = async () => {
      try {
        const res = await fetch(`/api/upcoming-events?id=${id}`);
  ...
  const onSubmit = async (data: SliderFormData) => {
    startTransition(async () => {
      try {
        ...
        router.push("/dashboard/"); // Redirect to slider list page.
      } catch (error) {
  ...
  return (
    <main className="min-h-[calc(100vh-5.2rem)] px-10 py-5">
      ...
      <h1 className="xl:text-xl font-bold">
            <TranslateText english_text="Create Upcoming Event" marathi_text="आगामी कार्यक्रम तयार करा">Edit Upcoming Event</TranslateText>
         </h1>
         <TranslateText  className="text-sm mt-2 text-muted-foreground" english_text="Fill in the details to create a new event." marathi_text="नवीन कार्यक्रम तयार करण्यासाठी तपशील भरा.">Fill in the details to edit the  event.</TranslateText>
         </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5 max-w-2xl">
        ...
        <Label htmlFor="e_title">
           ...
           <Input
            ...
            type="text"
            placeholder={
              lang === "english" ? "English title" : "इंग्रजी शीर्षक"
