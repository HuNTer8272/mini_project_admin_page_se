...
const Page = () => {
  ...
  const onSubmit = async (data: EventFormData) => {
    ...
    try {
      ...
      const response = await fetch("/api/about-events", {
        method: "POST",
  ...
  return (
    <main className="min-h-[calc(100vh-5.2rem)] px-10 py-5">
      ...
      <h1 className="xl:text-xl font-bold">
          <TranslateText english_text="Create Event" marathi_text="कार्यक्रम तयार करा"> Create Event</TranslateText>
        </h1>
       <TranslateText className="text-sm mt-2 text-muted-foreground" english_text="Fill in the details to create a new event." marathi_text="नवीन कार्यक्रम तयार करण्यासाठी तपशील भरा.">Fill in the details to create a new event.</TranslateText>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5 max-w-2xl">
        ...
        <Button type="submit" className="mt-5 px-8 py-2 rounded-full" disabled={isSaving}>
         {isSaving ? lang === "english"? "Saving..." : "सेव्ह करत आहे..." : lang === "english"? "Save" : "बदलावे जतन करा"}  
        </Button>
      </form>
    </main>
  );
