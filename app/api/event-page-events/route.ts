    ...
    try {
        ...
        if (id) {
        const event = await prisma.event_page_events.findUnique({
            where: { id: Number(id) },
        });
        if (!event)
        ...
        const event_page_events = await prisma.event_page_events.findMany();
        const decompressedevents = event_page_events.map((event) => ({
            ...event,
    ...
    try {
        ...
        const event = await prisma.event_page_events.create({
        data: {
        ...
        console.error("Error creating about_event record:", error);
        return NextResponse.json(
        { error: error instanceof Error ? error.message : error || "Failed to create about event" },
        { status: 500 }
        );
    }
    }
    ...
    try {
        ...
        const event = await prisma.event_page_events.delete({
    ...
    export async function PUT(request: Request) {
        ...
        const event = await prisma.event_page_events.update({
            where: { id: Number(data.id) },
            data: {
