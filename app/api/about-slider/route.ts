...
export async function GET(request: Request) {
  try {
    ...
    if (id) {
      const slider = await prisma.about_slider.findUnique({
        where: { id: Number(id) },
      });
      if (!slider)
      ...
      const about_sliders = await prisma.about_slider.findMany();
      const decompressedSliders = about_sliders.map((slider) => ({
        ...slider,
    ...
    console.error("Error fetching about_slider records:", e);
...
export async function POST(request: Request) {
  try {
    ...
    const slider = await prisma.about_slider.create({
      data: {
    ...
    console.error("Error creating about_slider record:", error);
...
export async function DELETE(request: Request) {
  try {
    ...
    const slider = await prisma.about_slider.delete({
      where: { id: Number(data.id) },
    });
    return NextResponse.json(slider, { status: 200 });
  } catch (error) {
    console.error("Error deleting about_slider record:", error);
    return NextResponse.json(
...
export async function PUT(request: Request) {
    ...
    const event = await prisma.about_slider.update({
        where: { id: Number(data.id) },
        data: {
